...
public class TagAliasOperatorHelper {
    ...
    public static int sequence = 555;
    /**
     * 增加
     */
            ...
            switch (msg.what) {
                case DELAY_SEND_ACTION:
                    if (msg.obj != null && msg.obj instanceof TagAliasBean) {
                        LogWrapper.i(TAG, "on delay time");
//                        sequence++;
                        ...
                        if (context != null) {
                            ...
                            LogWrapper.e(TAG, "#unexcepted - context was null");
                        }
                    } else {
                        LogWrapper.w(TAG, "#unexcepted - msg obj was incorrect");
                    }
                    break;
            }
        }
    ...
    public void handleAction(Context context, int sequence, TagAliasBean tagAliasBean) {
        ...
        if (tagAliasBean == null) {
            LogWrapper.w(TAG, "tagAliasBean was null");
            return;
        }
        ...
        if (tagAliasBean.isAliasAction) {
            switch (tagAliasBean.action) {
                ...
                case ACTION_SET:
                    ...
                    break;
                default:
                    LogWrapper.w(TAG, "unsupport alias action type");
                    return;
            }
        } else {
            switch (tagAliasBean.action) {
                ...
                case ACTION_CLEAN:
                    ...
                    break;
                default:
                    LogWrapper.w(TAG, "unsupport tag action type");
                    return;
            }
        }
    }
    ...
    private boolean RetryActionIfNeeded(int errorCode, TagAliasBean tagAliasBean) {
        if (!JPushUtil.isConnected(context)) {
            LogWrapper.w(TAG, "no network");
            return false;
        }
        //返回的错误码为6002 超时,6014 服务器繁忙,都建议延迟重试
        if (errorCode == 6002 || errorCode == 6014) {
            LogWrapper.d(TAG, "need retry");
            if (tagAliasBean != null) {
    ...
    public void onTagOperatorResult(Context context, JPushMessage jPushMessage) {
        ...
        LogWrapper.i(TAG, "action - onTagOperatorResult, sequence:" + sequence + ",tags:" + jPushMessage.getTags());
        LogWrapper.i(TAG, "tags size:" + jPushMessage.getTags().size());
        init(context);
        ...
        if (jPushMessage.getErrorCode() == 0) {
            LogWrapper.i(TAG, "action - modify tag Success,sequence:" + sequence);
            tagAliasActionCache.remove(sequence);
            String logs = getActionStr(tagAliasBean.action) + " tags success";
            LogWrapper.i(TAG, logs);
            JPushUtil.showToast(logs, context);
        } else {
    ...
    public void onCheckTagOperatorResult(Context context, JPushMessage jPushMessage) {
        int sequence = jPushMessage.getSequence();
        LogWrapper.i(TAG, "action - onCheckTagOperatorResult, sequence:" + sequence + ",checktag:" + jPushMessage.getCheckTag());
        init(context);
        //根据sequence从之前操作缓存中获取缓存记录
        TagAliasBean tagAliasBean = tagAliasActionCache.get(sequence);
        if (tagAliasBean == null) {
        ...
        if (jPushMessage.getErrorCode() == 0) {
            LogWrapper.i(TAG, "tagBean:" + tagAliasBean);
            tagAliasActionCache.remove(sequence);
    ...
    public void onAliasOperatorResult(Context context, JPushMessage jPushMessage) {
        int sequence = jPushMessage.getSequence();
        LogWrapper.i(TAG, "action - onAliasOperatorResult, sequence:" + sequence + ",alias:" + jPushMessage.getAlias());
        init(context);
        //根据sequence从之前操作缓存中获取缓存记录
        TagAliasBean tagAliasBean = tagAliasActionCache.get(sequence);
        if (tagAliasBean == null) {
        ...
        if (jPushMessage.getErrorCode() == 0) {
            LogWrapper.i(TAG, "action - modify alias Success,sequence:" + sequence);
            tagAliasActionCache.remove(sequence);
